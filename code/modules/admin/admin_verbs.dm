...
GLOBAL_LIST_INIT(admin_verbs_possess, list(/proc/possess, GLOBAL_PROC_REF(possess), /proc/release))
...
/client/proc/deadmin()
	...
	to_chat(src, span_interface("You deadmined yourself."))
...
/client/proc/readmin()
	...
	if(!A)
		...
		if (!A)
			var/msg = " Tried to readmin but they have no deadmin entry"
