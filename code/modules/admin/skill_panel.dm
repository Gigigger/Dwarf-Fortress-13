...
/datum/skill_panel/New(user, mob/target_user)//H can either be a client or a mob due to byondcode(tm)
	target = target_user
	...
	switch (action)
		if("add_skill")
			...
			for(var/skill_type in subtypesof(/datum/skill))
				if(target.get_skill(skill_type))
					continue
			...
			target.adjust_experience(selected_skill, 0)
		if ("remove_skill")
