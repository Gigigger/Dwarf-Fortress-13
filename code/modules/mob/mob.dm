...
/mob/proc/run_examinate(atom/examinify, force_examine_more=FALSE)
	...
	if(client)
		...
		if((isnull(client.recent_examines[examinify]) || client.recent_examines[examinify] < world.time) && !force_examine_more)
			result = examinify.examine(src)
			// very hacky but we don't want to show this in every examine
			if(LAZYLEN(examinify.examine_more(src)))
				result += "<br><i><a href='byond://?src=[REF(examinify)];examine_more=1'>Click for closer inspection.</a></i>"
			...
			if(!LAZYLEN(result)) // lol ..length
				result = list(span_notice("<i>You examine <b>[examinify]</b> closer, but find nothing of interest...</i>"))
	else
		result = examinify.examine(src) // if a tree is examined but no client is there to see it, did the tree ever really exist?

	if(result)
		to_chat(src, examine_block("[examinify.get_examine_string(src, TRUE)].", result.Join()))
