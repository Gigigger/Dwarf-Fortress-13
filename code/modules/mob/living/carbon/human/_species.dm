...
// Called when a carbon is getting qdel'd
/datum/species/proc/on_species_destroy(mob/living/carbon/human/C)
	return
...
/datum/species/proc/apply_damage(damage, damagetype = BRUTE, def_zone = null, blocked, mob/living/carbon/human/H, forced = FALSE, spread_damage = FALSE, wound_bonus = 0, bare_wound_bonus = 0, attack_type = BLUNT, attack_direction = null)
	...
	switch(damagetype)
		if(BRUTE)
			...
			if(BP)
				if(BP.receive_damage(damage_amount, 0, wound_bonus = wound_bonus, bare_wound_bonus = bare_wound_bonus, attack_type = attack_type, attack_direction = attack_direction, blocked = blocked))
		...
		if(BURN)
			...
			if(BP)
				if(BP.receive_damage(0, damage_amount, wound_bonus = wound_bonus, bare_wound_bonus = bare_wound_bonus, attack_type = attack_type, attack_direction = attack_direction, blocked = blocked))
					H.update_damage_overlays()
			else
