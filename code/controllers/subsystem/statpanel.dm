...
/datum/controller/subsystem/statpanels/proc/set_spells_tab(client/target, mob/target_mob)
	// I'm assuming the spells as presented here don't exist and were either completely cut from the code (be it accidental or not) or are coded differently.
	// I don't have the patience to look through that and fix that cause we don't even use spells right now - Maxsc
	// get_proc_holders() doesn't exist and I'm not sure what it's supposed to be here
