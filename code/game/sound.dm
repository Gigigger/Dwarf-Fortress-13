...
/proc/playsound(atom/source, soundin, vol as num, vary, extrarange as num, falloff_exponent = SOUND_FALLOFF_EXPONENT, frequency = null, channel = 0, pressure_affected = TRUE, ignore_walls = TRUE, falloff_distance = SOUND_DEFAULT_FALLOFF_DISTANCE, use_reverb = FALSE)
	if(isarea(source))
	...
	use_reverb = !(SSmapping.level_trait(source_z, ZTRAIT_ABOVE_GROUND))
	...
	for(var/P in listeners)
		...
		if(get_dist(M, turf_source) <= maxdistance)
			M.playsound_local(turf_source, soundin, vol, vary, frequency, falloff_exponent, channel, pressure_affected, S, maxdistance, falloff_distance, use_reverb=use_reverb)
...
/proc/get_sfx(soundin)
	if(istext(soundin))
		switch(soundin)
			...
			if("honkbot_e")
				soundin = pick('sound/items/bikehorn.ogg', 'sound/items/airhorn2.ogg', 'sound/misc/sadtrombone.ogg', 'sound/items/airhorn.ogg', 'sound/effects/reee.ogg',  'sound/items/WEEOO1.ogg', 'sound/voice/beepsky/iamthelaw.ogg', 'sound/voice/beepsky/creep.ogg','sound/magic/fireball.ogg' ,'sound/effects/pray.ogg', 'sound/voice/hiss1.ogg', 'sound/machines/ping.ogg', 'sound/weapons/flashbang.ogg', 'sound/weapons/bladeslice.ogg')
			if("goose")
