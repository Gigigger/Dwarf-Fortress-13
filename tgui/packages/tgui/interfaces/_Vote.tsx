...
export const _Vote = (props) => {
  const { data } = useBackend();
  const { mode, question, lower_admin } = data as any;
  ...
  return (
    <Window theme="dwarf" title={windowTitle} width={400} height={550}>
      <Window.Content>
        <Stack fill vertical>
...
const ChoicesPanel = (props, context) => {
  const { act, data } = useBackend();
  const { choices, selected_choice } = data as any;
...
const TimePanel = (props) => {
  const { act, data } = useBackend();
  const { lower_admin, time_remaining } = data as any;
