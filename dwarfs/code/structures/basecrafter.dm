...
/obj/structure/crafter/proc/get_highest_grade(minigame_grade=null)
	if(minigame_grade)
		return minigame_grade
	. = 1
...
/obj/structure/crafter/attack_hand(mob/user)
	if(busy)
		...
		var/res_grade = selected_recipe.start_crafting(user)

		// minigame was aborted
		if(!res_grade && selected_recipe.minigame)
			busy = FALSE
			return

		res_grade = get_highest_grade(res_grade)
		...
		if(user && !do_after(user, assembly_time, src))
			busy = FALSE
		...
		for(var/i in 1 to selected_recipe.result_amount)
			...
			if(length(_parts) > 1)
				O.grade_parts = _parts

			O.grade = res_grade
			O.update_stats()

		if(selected_recipe.affecting_skill)
			if(selected_recipe.minigame)
				var/mob/last_user = selected_recipe.minigame.last_user.resolve()
				if(last_user)
					last_user.adjust_experience(selected_recipe.affecting_skill, selected_recipe.exp_gain * res_grade)
			else
				user.adjust_experience(selected_recipe.affecting_skill, selected_recipe.exp_gain * res_grade)
		...
		selected_recipe = new selected_type(src)
