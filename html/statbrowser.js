...
function createStatusTab(name) {
	...
	button.innerHTML = "<span class='statpanel24x24 tabicon-"+getIconForTabByName(name)+"'></span>"
...
function getIconForTabByName(name) {
	switch (name) {
		case 'Status':
			return 'status';
		case 'IC':
...
function set_theme(which) {
	if (which == "light") {
		...
		document.body.className = "dwarf";
		document.documentElement.className = "dwarf";
	}
}
